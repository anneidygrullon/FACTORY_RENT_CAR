...
public class EntregaVehiculoController {
    ...
    private void cargarEntregas() {
        ...
        try (Connection con = conexion.establecerConexion();
            ...
            System.out.println("Entregas cargadas: " + listaControles.size());
        } catch (SQLException e) {
            JOptionPane.showMessageDialog(null, "Error cargando entregas: " + e.getMessage());
        }
    }
    ...
    private void actualizarEntrega(ActionEvent event) {
        ...
        try {
            ...
            String sql = "UPDATE TBL_CONTROL_VEHI SET fecha=?, nivel_combustible=?, tipo=?, fk_pk_id_direccion=? WHERE id_control=?";
            try (Connection con = conexion.establecerConexion();
                 PreparedStatement ps = con.prepareStatement(sql)) {
